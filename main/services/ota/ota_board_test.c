...
static bool ota_board_test_wait_manifest_result(void)
{
    ...
    while (xTaskGetTickCount() < deadline)
    {
        ...
        if (ota_service_get_snapshot(&snapshot) == ESP_OK)
        {
            ...
            {
                ESP_LOGE(TAG, "update check rejected: state=%s err=%s",
...
static void ota_board_test_task(void *arg)
{
    ...
    esp_err_t ret = ota_service_request_check();
    if (ret != ESP_OK)
    {
        ESP_LOGE(TAG, "check command rejected: %s", esp_err_to_name(ret));
        goto done;
    }
    ...
    ret = ota_service_request_download();
