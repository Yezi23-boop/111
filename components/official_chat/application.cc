...
esp_err_t Application::SetEventCallback(official_chat_event_callback_t callback,
  ...
  Schedule([this]() { ReplayEventSnapshot(); });
  return ESP_OK;
}
...
void Application::HandleWakeWordDetectedEvent() {
  ...
  if (state == DeviceState::kIdle) {
    ...
    if (!protocol_->IsAudioChannelOpened()) {
      SetDeviceState(DeviceState::kConnecting);
      Schedule(
          [this, wake_word]() { ContinueWakeWordInvoke(wake_word); });
      return;
    }
...
void Application::StartActivationTask() {
  ...
  if (xTaskCreate(
          ...
          kActivationTaskName, kActivationTaskStackBytes, this, 5,
          &activation_task_handle_) != pdPASS) {
    ESP_LOGE(kTag, "failed to create activation task");
...
void Application::ActivationTask() {
  ...
  const AssetsRuntime::Result assets_result =
      assets_runtime_.CheckAndApplyPendingDownload(
          [this](int progress, size_t speed) {
            Schedule([this, progress, speed]() {
              last_assets_progress_.progress = progress;
              last_assets_progress_.speed_bytes_per_sec = speed;
              last_assets_progress_.valid = true;
              EmitProgressEvent(OFFICIAL_CHAT_EVENT_ASSETS_PROGRESS, progress,
                                speed);
            });
          });
  if (assets_result.attempted && !assets_result.applied) {
    Schedule([this, message = assets_result.message,
              error = assets_result.error]() {
      EmitErrorEvent(error, message);
    });
  }

  const esp_err_t version_err = ota_->CheckVersion();
  if (version_err != ESP_OK) {
    Schedule([this, version_err]() {
      EmitErrorEvent(version_err, "ota check version failed");
      if (event_group_ != nullptr) {
  ...
  if (ota_->HasActivationCode()) {
    Schedule([this]() {
      EmitMessageEvent(OFFICIAL_CHAT_EVENT_ACTIVATION_MESSAGE,
                       ota_->GetActivationMessage());
      EmitMessageEvent(OFFICIAL_CHAT_EVENT_ACTIVATION_CODE,
  ...
  if (ota_->HasActivationChallenge()) {
    ...
    for (int attempt = 0; started_ && attempt < kMaxActivationAttempts;
         ++attempt) {
      ESP_LOGI(kTag, "Activating... %d/%d", attempt + 1, kMaxActivationAttempts);
      const esp_err_t activate_err = ota_->Activate();
      if (activate_err == ESP_OK) {
        activation_succeeded = true;
        break;
      }
      if (attempt == kMaxActivationAttempts - 1) {
        Schedule([this, activate_err]() {
          EmitErrorEvent(activate_err, "activation failed");
        });
      }
      vTaskDelay(activate_err == ESP_ERR_TIMEOUT ? pdMS_TO_TICKS(3000)
                                                 : pdMS_TO_TICKS(10000));
    }
  ...
  if (started_ && ota_->HasNewVersion()) {
    Schedule([this]() { SetDeviceState(DeviceState::kUpgrading); });
    const bool upgraded = ota_->StartUpgrade([this](int progress, size_t speed) {
      Schedule([this, progress, speed]() {
        last_upgrade_progress_.progress = progress;
        last_upgrade_progress_.speed_bytes_per_sec = speed;
        last_upgrade_progress_.valid = true;
        if (event_group_ != nullptr) {
          xEventGroupSetBits(event_group_, kMainEventUpgradeProgress);
        }
      });
    });
    if (upgraded) {
    ...
    Schedule([this]() {
      EmitErrorEvent(ESP_FAIL, "firmware upgrade failed");
      SetDeviceState(DeviceState::kActivating);
    });
  }
