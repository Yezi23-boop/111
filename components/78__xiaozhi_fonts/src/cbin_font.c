...
lv_font_t *cbin_font_create(uint8_t *bin_addr) {
    ...
    lv_font_fmt_txt_dsc_t *dsc =
        (lv_font_fmt_txt_dsc_t *)malloc_cpy(bin_addr, sizeof(lv_font_fmt_txt_dsc_t));
    font->dsc = dsc;
    ...
    if (dsc->cmap_num) {
        ...
        dsc->cmaps = (lv_font_fmt_txt_cmap_t *)malloc(
            sizeof(lv_font_fmt_txt_cmap_t) * dsc->cmap_num);
    ...
    if (dsc->kern_dsc) {
        ...
        if (dsc->kern_classes == 1) {
            lv_font_fmt_txt_kern_classes_t *kcl =
                (lv_font_fmt_txt_kern_classes_t *)malloc_cpy(
                    kern_addr, sizeof(lv_font_fmt_txt_kern_classes_t));
            dsc->kern_dsc = kcl;
            ...
            lv_font_fmt_txt_kern_pair_t *kp =
                (lv_font_fmt_txt_kern_pair_t *)malloc_cpy(kern_addr,
