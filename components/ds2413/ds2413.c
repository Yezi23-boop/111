...
{
    return family_code == DS2413_FAMILY_CODE ||
           family_code == DS2413_CLONE_FAMILY_CODE ||
           family_code == DS2413_BOARD_FAMILY_CODE;
}

static esp_err_t ds2413_find_by_index_impl(onewire_bus_handle_t bus, size_t index,
...
static esp_err_t ds2413_select_device(const ds2413_device_t *device)
{
    ...
    ESP_RETURN_ON_ERROR(onewire_bus_write_bytes(device->bus, &match_cmd, 1), TAG,
                        "write MATCH ROM failed");
    ESP_RETURN_ON_ERROR(onewire_bus_write_bytes(device->bus,
                                                (const uint8_t *)&device->address,
                                           ...
                                           ds2413_device_t *device)
{
    ...
    while ((search_ret = onewire_device_iter_get_next(iter, &next_device)) == ESP_OK)
    {
    ...
    if (del_ret != ESP_OK)
    {
        ESP_LOGW(TAG, "delete device iterator returned 0x%x while no device matched",
                 (int)del_ret);
    }
    return ret;
}
...
esp_err_t ds2413_read_state(const ds2413_device_t *device, ds2413_state_t *state)
{
    ...
    uint8_t raw = 0;
    ESP_RETURN_ON_ERROR(onewire_bus_write_bytes(device->bus, &read_cmd, 1), TAG,
                        "write PIO_ACCESS_READ failed");
                             ...
                             ds2413_state_t *verified_state)
{
    ...
    const uint8_t inverted_state = (uint8_t)~state_byte;
    uint8_t ack = 0;
    uint8_t raw = 0;

    ESP_RETURN_ON_ERROR(onewire_bus_write_bytes(device->bus, &write_cmd, 1), TAG,
                        "write PIO_ACCESS_WRITE failed");
    ESP_RETURN_ON_ERROR(onewire_bus_write_bytes(device->bus, &state_byte, 1), TAG,
                        "write DS2413 state failed");
    ESP_RETURN_ON_ERROR(onewire_bus_write_bytes(device->bus, &inverted_state, 1), TAG,
    ...
    ds2413_state_t *out_state = verified_state != NULL ? verified_state : &temp_state;
