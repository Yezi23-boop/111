...
scope: repo
owners: main/app/app_main.c, main/app/hardware_init.c, main/ui/lvgl_task.c, main/services/power/power_policy.c, main/services/runtime/runtime_coordinator.c, main/services/runtime/safety_monitor_policy.c, main/services/safety/safety_monitor_session.c, main/services/network/network_service.c, main/services/official_chat_service.c, components/audio_codec, components/network_manager, main/features/danger_detection/danger_detection_service.c, main/features/alerts/app_alert_manager.c
triggers: runtime owner, owner contract, framework, 启动阶段, 资源 owner, 后台能力, power_policy, runtime_coordinator, safety_monitor_policy, safety monitor, official_chat, ui_refresh_policy
...
- 不新增拥有真实资源的 `runtime lease` 仲裁中心；跨 owner 重复协议只由窄 `runtime_coordinator` 处理，资源可用性仍通过 owner snapshot、blocker 和 `power_policy` budget 表达。
...
当前运行时骨架采用 `FreeRTOS owner snapshot + power_budget + runtime_coordinator protocol`，不是中心化资源管家。
...
- 长期服务用 task 表达，例如 `power_service`、`power_policy`、`network_service`、`official_chat_service`、`runtime_coordinator`、`safety_monitor_policy`、`sleep_coordinator`。
...
| Service Managers | `runtime_coordinator` + `safety_monitor_policy` | 创建控制面 task、等待 `ui_first_frame_ready`、协调强前台交接并计算 Safety Monitor 目标态 | 直接创建/销毁业务资源、理解 owner 内部状态机 | `boot_stage: managers_ready`、`safety_monitor_gate_ready` |
...
| 跨 owner 协调事实 | `runtime_coordinator` | 强前台 owner、可抢占后台 participant、UI/诊断 | 只拥有注册、generation、deadline、ACK 和当前强前台事实，不拥有真实资源 |
| 整机资源预算 | `power_policy` | `safety_monitor_policy`、service/domain owner | 发布 `ACTIVE / STANDBY / CHARGING / LOW_BATTERY_WARN / MAINTENANCE` 等预算，不直接操作具体资源 |
| 后台安全监听开关 | `safety_monitor_policy` | UI、`safety_monitor_session` | 保存用户意图，合成 `should_run`，不拥有危险识别 runtime |
| Safety Monitor 会话生命周期 | `safety_monitor_session` | `safety_monitor_policy` | 把 `should_run` 翻译成 start/stop、FAILED 恢复、退避和运行确认 |
...
| AI 前台音频 | `official_chat_service` | `safety_monitor_policy` | AI 对话页前台期间声明 foreground audio，Safety Monitor 暂停；退出后按用户开关恢复 |
...
Safety Monitor 由独立 policy owner 管理，并作为可抢占后台 participant 注册到 coordinator。

```text
危险识别页 UI 开关
  -> safety_monitor_policy_set_enabled()
  -> safety_monitor_policy task notification
  -> safety_monitor_policy 读取 power budget + 音频资源快照 + coordinator block
...
- Safety Monitor 默认按用户意图开启，但仍必须等 `ui_first_frame_ready`
  后由 `safety_monitor_policy` 按 power budget、麦克风 owner 与 coordinator block 合成
...
| 跨 owner 协议 | `runtime_coordinator` | 注册 participant、串行 request/transition generation、deadline、ACK 和强前台事实 | 包含业务 service 头文件、直接 stop/deinit 资源、解释 OTA/网络/模型状态 |
| 后台目标态 | `safety_monitor_policy` | 保存用户意图，通过 task notification 响应用户开关/前台音频/power budget/coordinator 变化，合成 Safety Monitor 是否应运行 | 变成模型 owner、提醒策略 owner、音频仲裁器或通用任务调度器 |
...
2026-07-31 复查确认 Hermes、official chat、network provisioning 与 OTA 已重复实现同一套 quiesce/gate 生命周期，达到新增共享协议层门槛。该证据只授权 `runtime_coordinator` 协议层，不授权大而全 `ResourceManager`、`session_router` 或默认 `ui_manager`。

### 新后台能力接入条件

新能力只有会真实争抢关键资源并同时满足以下条件，才允许注册到 `runtime_coordinator`：
...
- 新增 runtime lease、通用资源账本或让 coordinator 绕过 owner ACK 直接操作资源。
- `runtime_coordinator` 包含业务 service 头文件、解释业务状态枚举或直接执行 stop/deinit。
- `safety_monitor_policy` 变成通用任务调度器、模型后端 owner、提醒策略 owner 或音频仲裁器。
