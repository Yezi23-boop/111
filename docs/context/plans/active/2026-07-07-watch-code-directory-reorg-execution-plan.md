...
memory_type: task
scope: task
owners: docs/context/plans/active/2026-07-07-watch-code-directory-reorg-execution-plan.md, main/CMakeLists.txt, main/services, main/features, main/ui, tools, tests, scripts/context/check_layering.py
...
- 下一步最小动作：阶段 1，先补目录边界 source tests / `check_layering.py`，特别是 UI generated 禁止 include service/driver、`tools/ui_preview` 独立分类、Memory Watch 文件归属规则。
- 阶段 1 通过后，再执行阶段 2：迁移 `main/ui/agent_preview -> tools/ui_preview`。
