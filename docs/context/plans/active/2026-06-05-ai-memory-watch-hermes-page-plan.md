...
- 当前 Cloudflare Tunnel 仍为 `down`，公网 `watch.934000.xyz` 返回 Cloudflare `530 / 1033`，原因是本机/服务器尚未运行 `cloudflared` connector；新增 `deploy/start_cloudflared_connector.ps1` 后，connector token 预期放在仓库外 `D:\Docker_data\hermes\cloudflared_tunnel_token.txt`，脚本用只读挂载和 `TUNNEL_TOKEN_FILE` 启动独立 `ai-memory-watch-cloudflared` 容器，不把 token 放进 Docker 命令行。
...
- 2026-06-09 公网联调准备闭环：本机 `smoke_test.ps1` mock 上传返回 `voice_status=done/action=memory_saved/field_count=7`，真实 `make_tts_sample.ps1 -> smoke_test.ps1 -UseRealAsr` 返回 `asr_mode=real/voice_status=done/action=memory_saved/field_count=7`；`runtime_status.ps1 -BaseUrl https://watch.934000.xyz -SkipDocker -SkipHermesApi -SkipServiceHealth -AssertPrivateNotExposed` 当前因 Cloudflare `530/1033` 未通过，等待 connector token 文件后启动 Tunnel。COM3 串口验证当前固件仍为 `111` 主应用，自动连接 Wi-Fi `li` 并拿到 `192.168.41.11`，`memory_watch_ready` 已出现，但 `watch endpoint NVS config not found`，后续需通过 SoftAP 门户或自动化入口写入 `https://watch.934000.xyz` 的 watch endpoint 配置。
