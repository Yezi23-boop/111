...
scope: repo
owners: components/wifi_control, components/network_manager, main/services/network_service
...
若后续发现连接切换场景异常，最小回退点是 `components/wifi_control/src/wifi_control.c` 中 `wifi_control_runtime_needs_disconnect_before_connect()` 与 `wifi_control_connect()` 的条件断开逻辑。但不建议回退断连日志字段，因为它们是后续定位 reason/suppress/retry 的关键证据。
