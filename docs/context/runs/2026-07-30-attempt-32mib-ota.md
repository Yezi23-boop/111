...
- 分支/工作区状态：未记录
- 设备/串口/板型：COM7，ESP32-S3 QFN56，32MiB Flash，8MiB PSRAM
- 关键前置条件：未记录
...
- 删除未使用的 audio 分区；model 扩至 0x1E0000，并完整烧录 COM7。烧录前擦除 0x1E20000..0x2000000，避免旧 audio 内容残留在新 model 分区。
...
- COM7 启动日志确认 audio 标签已不存在；model 位于 0x1E20000，长度 0x1E0000；resources LittleFS 成功挂载（used=3346432）；30 秒采集无 panic。
- 与预期不一致的点：
- 未记录

## 结论

- 本次可以确认的事实：32MiB 可容纳 12MiB 双 OTA 槽；当前 resources 固定 4MiB、assets 保留 2MiB、audio 已删除、model 为 1.875MiB，分区表恰好填满 Flash。正式固件已固定使用 ESP-DL，Edge Impulse 交通声音组件和 manual_v7_1s 模型不再编译或链接。
- 仍然不能确认的事实：
...
- 下一轮仍需补证据的边界：
- 实现独立 HTTPS ota_service 与启动确认；后续若改动分区表，仍需采用完整分区烧录并按变动范围显式擦除迁移的数据分区。
