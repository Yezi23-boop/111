...
class MainScreenBleToggleSourceTests(unittest.TestCase):
    ...
    def test_main_dropdown_controller_syncs_ble_state_and_shows_toast(self) -> None:
        ...
        self.assertIn("network_service_set_ble_enabled(", source)
        self.assertIn("network_service_get_snapshot(", source)
        self.assertIn("ble_desired_enabled", source)
        self.assertIn("ble_applied_enabled", source)
        self.assertIn("ble_transition_pending", source)
        self.assertNotIn("network_manager_is_ble_active()", source)
        self.assertNotIn("network_manager_is_ble_enabled()", source)
        self.assertNotIn("network_manager_set_ble_enabled(", source)
        self.assertNotIn("foreground_runtime_gate_acquire", source)
        self.assertNotIn("foreground_runtime_gate_release", source)
