...
class DangerDetectionVisualSourceTests(unittest.TestCase):
    def test_view_and_controller_use_fullscreen_color_states(self) -> None:
        ...
        self.assertIn("danger_detection_view_visual_state_t", view_header)
        self.assertIn("danger_detection_view_set_visual_state", view_header)
        self.assertIn("top_bar", view_source)
        self.assertIn("lv_obj_set_size(view->top_bar, LV_PCT(100), 60)", view_source)
        ...
        self.assertIn("DANGER_DETECTION_VIEW_VISUAL_STATE_IDLE", view_source)
        self.assertIn("lv_color_hex(0xffffff)", view_source)
        self.assertIn("lv_palette_main(LV_PALETTE_RED)", view_source)
        self.assertIn("danger_detection_view_set_visual_state", controller_source)
