...
class ImuServiceSourceTests(unittest.TestCase):
    ...
    def test_board_imu_config_centralizes_only_current_board_facts(self) -> None:
        ...
        self.assertIn("手表右侧 / USB 侧朝上 -> QMI8658C `-X`", header)
        self.assertIn("手表左侧朝上 -> QMI8658C `+X`", header)
        self.assertIn("手表顶部朝上 -> QMI8658C `+Y`", header)
        self.assertIn("手表底部朝上 -> QMI8658C `-Y`", header)
    ...
    def test_app_main_starts_imu_service_by_default(self) -> None:
        ...
        self.assertNotIn("#if 0", nearby_source)
    ...
    def test_header_exposes_config_snapshot_getter_contract(self) -> None:
        ...
        self.assertIn("#define IMU_SERVICE_WINDOW_FRAME_COUNT 200U", header)
        self.assertIn("#define IMU_SERVICE_WINDOW_PUBLISH_STRIDE_FRAMES 50U", header)
    ...
    def test_service_uses_imu_sensor_and_owns_gpio21_isr(self) -> None:
        ...
        self.assertIn("imu_service_build_accel_window", source)
    ...
    def test_service_sampling_path_does_not_touch_ui_or_sleep(self) -> None:
        source = IMU_SERVICE_SOURCE.read_text(encoding="utf-8")
        self.assertIn("started: sampling_50hz", source)
        forbidden_tokens = (
